...
def check_zero_temperature_physicality(
    ...
    h = 1.0e-3 * params.v
    f0 = float(
        V_effective_potential(params.v, 0.0, params, include_zeroT_loops=include_zeroT_loops, include_thermal=False,
                              include_daisy=False, include_glauber=include_glauber))
    fp = float(
        V_effective_potential(params.v + h, 0.0, params, include_zeroT_loops=include_zeroT_loops, include_thermal=False,
                              include_daisy=False, include_glauber=include_glauber))
    fm = float(
        V_effective_potential(params.v - h, 0.0, params, include_zeroT_loops=include_zeroT_loops, include_thermal=False,
                              include_daisy=False, include_glauber=include_glauber))
...
def example_A1_zero_temperature_potential(
    ...
    if min_full is not None:
        ax.scatter([min_full[0]], [min_full[1]], s=35, color="#2ca02c", zorder=5,
                   label=rf"sampled min $\phi\simeq{min_full[0]:.1f}$ GeV")
...
def example_B1_physicality_report(
    ...
    report = check_zero_temperature_physicality(params, include_zeroT_loops=False,
                                                include_glauber=params.use_glauber_measure)
...
def example_C1_finite_temperature_shapes(
        ...
        *,
        output_dir: str | Path,
        temperatures: Sequence[float] = (0.0, 75.0, 100.0, 125.0, 180),
        phi_max: float | None = None,
        n_phi: int = 2000,
        show: bool = False,
        include_zeroT_loops: bool = True,
...
def _main_transition_objects(summary: Mapping[str, Any]) -> tuple[Any, Any, Any, Any, Any, dict[str, Any]]:
    ...
    return main, phases[false_key], phases[true_key], false_key, true_key, dict(
        summary.get("key_temperatures", {}) or {})
...
def example_E1_temperature_scales(summary: dict[str, Any], *, output_dir: str | Path,
    ...
    ax.imshow(band, extent=(max(0.0, lo - margin), hi + margin, 0.0, 1.0), origin="lower", aspect="auto", cmap="plasma",
              alpha=0.7)
...
def example_E2_phi_min_vs_temperature(summary: dict[str, Any], *, output_dir: str | Path,
                                      show: bool = False) -> Path | None:
    try:
...
def example_E3_potential_at_key_temperatures(summary: dict[str, Any], *, output_dir: str | Path,
                                             phi_max: float | None = None, show: bool = False) -> Path | None:
    V_XT = summary.get("V_XT", None)
    if V_XT is None:
        return None
    key_T = summary.get("key_temperatures", {}) or {}
    temps = [("Tn", r"$T_n$", key_T.get("Tn")), ("Tc", r"$T_c$", key_T.get("Tc")),
             ("Ts_true", r"$T_s^{\rm true}$", key_T.get("Ts_true")),
             ("Ts_false", r"$T_s^{\rm false}$", key_T.get("Ts_false"))]
...
def example_E4_free_energy_difference(summary: dict[str, Any], *, output_dir: str | Path,
                                      show: bool = False) -> Path | None:
    try:
        _, false_phase, true_phase, *_rest, key_T = _main_transition_objects(summary)
    except RuntimeError as err:
...
def _make_phi_only_potential_at_Tn(summary: Mapping[str, Any], *, Tn: float) -> Callable[
...
def build_bounce_solution_at_Tn(summary: dict[str, Any], *, phitol: float = 1e-5, thinCutoff: float = 1e-4,
                                npoints: int = 800, xguess: float | None = None) -> dict[str, Any]:
    inputs = _extract_bounce_inputs_from_summary(summary)
    V_phi = _make_phi_only_potential_at_Tn(summary, Tn=inputs["Tn"])
    inst = SingleFieldInstanton(phi_absMin=inputs["phi_true"], phi_metaMin=inputs["phi_false"], V=V_phi, alpha=2,
                                phi_eps=1e-3)
    ...
    bounce = {**inputs, "V_phi": V_phi, "instanton": inst, "profile": profile, "action_breakdown": br,
              "S3": float(br.S_total), "S3_over_T": float(br.S_total) / inputs["Tn"]}
...
def example_F1_bounce_potential_geometry(summary: dict[str, Any], *, output_dir: str | Path,
                                         show: bool = False) -> Path | None:
    try:
    ...
    points = [(inst.phi_metaMin, r"$h_f$", BOUNCE_COLORS["false"]), (inst.phi_absMin, r"$h_t$", BOUNCE_COLORS["true"]),
              (inst.phi_bar, r"$h_{\rm bar}$", BOUNCE_COLORS["bar"]), (phi_top, r"$h_{\rm top}$", BOUNCE_COLORS["top"]),
              (phi0, r"$h_0$", BOUNCE_COLORS["phi0"])]
    for x, lab, col in points:
        if np.isfinite(x):
            ax.scatter([x], [float(inst.V(x)) - Vfalse], s=54, color=col, edgecolor="black", linewidth=0.35, zorder=5,
                       label=lab)
...
def example_F3_bubble_slice(summary: dict[str, Any], *, output_dir: str | Path, n_grid: int = 360,
                            show: bool = False) -> Path | None:
    try:
        b = _get_bounce(summary)
    except RuntimeError as err:
    ...
    im = ax.imshow(PHI, origin="lower", extent=(x.min(), x.max(), y.min(), y.max()), cmap="viridis",
...
def example_F4_action_diagnostics(summary: dict[str, Any], *, output_dir: str | Path,
                                  show: bool = False) -> Path | None:
    try:
        b = _get_bounce(summary)
    except RuntimeError as err:
...
def run_bounce_examples(summary: dict[str, Any], *, output_dir: str | Path, show: bool = False,
                        xguess: float | None = None, phitol: float = 1e-5, thinCutoff: float = 1e-4,
                        npoints: int = 800) -> dict[str, Path | None]:
...
def compute_gw_spectrum_summary(
    ...
    beta_over_H = float(beta_over_H_override) if beta_over_H_override is not None else float(
        gw_calc.beta_over_H(T_star, dT))
    f_mHz = np.logspace(np.log10(f_min_mHz), np.log10(f_max_mHz), int(n_freq))
    spectra = gw_calc.omega_total_h2(f_mHz, alpha=alpha, beta_over_H=beta_over_H, T_star=T_star, g_star=g_star, v_w=v_w,
                                     include_sw=include_sw, include_turb=include_turb, include_coll=include_coll,
                                     epsilon_turb=epsilon_turb, kappa_coll=kappa_coll)
    spectra_no_turb = gw_calc.omega_total_h2(f_mHz, alpha=alpha, beta_over_H=beta_over_H, T_star=T_star, g_star=g_star,
                                             v_w=v_w, include_sw=include_sw, include_turb=include_turb,
                                             include_coll=include_coll, epsilon_turb=0.0, kappa_coll=kappa_coll)
    spectra_max_turb = gw_calc.omega_total_h2(f_mHz, alpha=alpha, beta_over_H=beta_over_H, T_star=T_star, g_star=g_star,
                                              v_w=v_w, include_sw=include_sw, include_turb=include_turb,
                                              include_coll=include_coll, epsilon_turb=1.0, kappa_coll=kappa_coll)
    peaks = {}
    ...
    summary["gw_scales"] = {"gw_T_star_GeV": T_star, "gw_alpha": alpha, "gw_beta_over_H": beta_over_H, "gw_v_w": v_w,
                            "gw_g_star": g_star}
    summary["gw"] = {"f_mHz": f_mHz, "f_Hz": 1e-3 * f_mHz, "spectra": spectra, "spectra_no_turb": spectra_no_turb,
                     "spectra_max_turb": spectra_max_turb, "peaks": peaks,
                     "parameters": {"g_star": g_star, "v_w": v_w, "epsilon_turb": epsilon_turb,
                                    "frequency_unit": "mHz"}}
...
def example_G2_gw_total_with_sensitivities(summary: dict[str, Any], *, output_dir: str | Path,
                                           show: bool = False) -> Path | None:
    gw = summary.get("gw")
    if gw is None:
        return None
    ...
    if np.any(mask):
        ax.fill_between(f[mask], no[mask], mx[mask], facecolor="none", edgecolor=purple, hatch="////", linewidth=0.0,
                        alpha=0.9, label=r"$0\leq\epsilon_{\rm turb}\leq1$")
    ax.set_xlabel(r"$f$ [mHz]")
    ax.set_ylabel(r"$h^2\Omega_{\rm GW}(f)$")
...
def gather_thesis_diagnostics(summary: dict[str, Any], *, output_dir: str | Path, basename: str = "thesis_diagnostics",
                              label: str = "thesis_run") -> dict[str, Any]:
    ...
    if "bounce" in summary:
        ...
        try:
            diagnostics["bounce_diagnostics"] = gather_diagnostics(b["instanton"], b["profile"], label=label,
...
def run_all_examples(
    ...
    saved["A1"] = example_A1_zero_temperature_potential(params, output_dir=output_dir, show=show,
                                                        include_zeroT_loops=include_zeroT_loops)
    ...
    if C1_temperatures is None:
        temps = [0.0, 75.0, 100.0, 125.0, 180]
    ...
    if run_finite_temperature:
        ...
        saved["E3"] = example_E3_potential_at_key_temperatures(finite_summary, output_dir=output_dir,
                                                               phi_max=E3_phi_max, show=show)
        if finite_summary.get("main_transition", None) is not None:
            ...
            if run_gw:
                try:
                    ...
                    saved["G2"] = example_G2_gw_total_with_sensitivities(finite_summary, output_dir=output_dir,
                                                                         show=show)
                except Exception as err:
                    print("[runner] GW examples skipped:", repr(err))
            if save_diagnostics_table:
                gather_thesis_diagnostics(finite_summary, output_dir=output_dir,
    ...
    return {"output_dir": output_dir, "parameters": asdict(params), "figures": saved,
            "finite_temperature_summary": finite_summary}

def run_article_checks() -> None:
    run_all_examples(
        c6_over_f2_TeV2=2.0,
        c8_over_f4_TeV4=2.0,
        f_GeV=1000.0,
        ...
        run_tag="article_checks",
        base_dir="00_article_checks",
        show=False,
...
def run_standard_model_page() -> None:
    run_all_examples(
        c6_over_f2_TeV2=0.0,
        c8_over_f4_TeV4=0.0,
        f_GeV=1000.0,
        ...
        run_tag="SM_reference2",
        base_dir="01_standard_model",
        show=False,
        ...
        include_scalar_thermal=True,
        include_daisy=False,
        C1_temperatures=[0.0, 75.0, 100.0, 125.0, 160, 180],
        run_finite_temperature=True,
        T_min=1.0,
        T_max=250.0,
        phi_scan_range=(-10, 1000.0),
        n_phi_scan=1400,
        n_T_seeds=3,
        deltaX_target=0.1,
        phitol=1e-5,
...
EFT68_REPRESENTATIVE_POINTS = [
    ("near_weak_edge", 1.7, 2.0),
    ("weak_nucleating", 2.7, 2.0),
    ("intermediate", 3.0, 2.0),
    ("strong_detectable", 3.3, 2.0),
    ("near_no_nucleation", 3.7, 2.0),
]


def run_eft68_representative_pages() -> None:
    for role, c6, c8 in EFT68_REPRESENTATIVE_POINTS:
        run_all_examples(
            c6_over_f2_TeV2=c6,
            c8_over_f4_TeV4=c8,
            f_GeV=1000.0,
            ...
            run_tag=f"EFT68_{role}",
            base_dir="02_EFT68_representative_pages",
            show=False,
...
def run_eft68_grid_scan() -> None:
    c6_values = np.round(np.linspace(0.8, 4.2, 23), 4)
    c8_values = np.round(np.linspace(-5.0, 10.0, 26), 4)

    for c6 in c6_values:
        for c8 in c8_values:
            tag = (
                f"EFT68_grid_"
                f"c6_{_format_value_for_path(c6)}_"
                f"c8_{_format_value_for_path(c8)}"
            )
...
if __name__ == "__main__":
    # 0. Article checks: run once.
    #run_article_checks()

    # 1. Standard Model control page.
    run_standard_model_page()

    # 2. A few complete EFT68 pages.
    #run_eft68_representative_pages()

    # 3. Full EFT68 grid for future maps.
    #run_eft68_grid_scan()

    # 4. Pure Gláuber grid.
    # run_glauber_pure_grid_scan()

    # 5. Pure Gláuber line scan at Lambda=1000 GeV.
    # run_glauber_lambda1000_line_scan()
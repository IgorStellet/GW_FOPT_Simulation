...
def V_paper(phi: np.ndarray | float, T: np.ndarray | None=None | float , C: float = 3.0, Lambda: float = 1000.0,
            finiteT: bool =False,
...
# Finite-difference step in φ (GeV). Order-of-magnitude: ~10^-3 v
_DPHI_FD: float = 1.0e-5 * _VEW  # _VEW already defined in your module


def _V_paper_phi_wrapper(
    X: np.ndarray,
    ...
    finiteT: bool = False,
    include_daisy: bool = True,
    real_cont: bool = False,
) -> np.ndarray:
    """
    Internal helper: view V_paper as a function of a single variable φ (Ndim=1),
    suitable for gradientFunction / hessianFunction.

    Parameters
    ----------
    X : ndarray
        Array whose last axis is the 1D field coordinate φ. Extra axes are
        treated as batch dimensions (including stencil indices).
    T, C, Lambda, finiteT, include_daisy, real_cont :
        Passed through to V_paper.

    Returns
    -------
    V : ndarray
        Same shape as X with the last axis removed.
    """
    X = np.asarray(X, dtype=float)
    # Last axis contains the single field component φ.
    phi = X[..., 0]  # shape (..., P) for some stencil size P
    return V_paper(
        phi,
        T=T,
        C=C,
...
# Build high-order finite-difference operators in φ once.
# Ndim=1 → the "vector" has only the φ component.
_dV_paper_dphi_fd = gradientFunction( _V_paper_phi_wrapper, eps=_DPHI_FD, Ndim=1, order=4)

_d2V_paper_dphi2_fd = hessianFunction(_V_paper_phi_wrapper, eps=_DPHI_FD, Ndim=1, order=4)
...
def dV_paper_dphi(
    ...
    """
    First derivative ∂V_paper/∂φ evaluated at (φ, T).

    Uses a 4th-order central finite-difference stencil along φ via gradientFunction.

    Parameters
    ----------
    phi : array_like
        Field value(s) φ (GeV) where the derivative is evaluated.
    T : float or array_like, optional
        Temperature(s) in GeV. For finiteT=True, T must be > 0. For finiteT=False,
        T is ignored.
    C, Lambda, finiteT, include_daisy, real_cont :
        Same meaning as in V_paper.

    Returns
    -------
    dV_dphi : ndarray
        Same shape as `phi`; ∂V/∂φ in GeV^3.
    """
    phi_arr = np.asarray(phi, dtype=float)
    # gradientFunction for Ndim=1 espera shape (..., 1)
    x = phi_arr[..., None]  # adiciona eixo de dimensão 1
    grad = _dV_paper_dphi_fd(
        x, T, C, Lambda, finiteT, include_daisy, real_cont
    )  # shape (..., 1)
    return grad[..., 0]
...
def d2V_paper_dphi2(
    ...
    """
    Second derivative ∂²V_paper/∂φ² evaluated at (φ, T).

    Uses hessianFunction with Ndim=1, de modo que o único elemento não trivial
    é H[..., 0, 0].
    ...
    phi : array_like
        Field value(s) φ (GeV).
    T : float or array_like, optional
        Temperature(s) em GeV. Para finiteT=True, T > 0.
    C, Lambda, finiteT, include_daisy, real_cont :
        Mesma interpretação física que em V_paper.

    Returns
    -------
    d2V_dphi2 : ndarray
        Mesma forma de `phi`; ∂²V/∂φ² em GeV^2.
    """
    phi_arr = np.asarray(phi, dtype=float)
    x = phi_arr[..., None]
    H = _d2V_paper_dphi2_fd(
        x, T, C, Lambda, finiteT, include_daisy, real_cont
    )  # shape (..., 1, 1)
    return H[..., 0, 0]


def d2V_paper_dphi_dT(
    phi: np.ndarray | float,
    T: np.ndarray | float,
    C: float = 3.0,
    Lambda: float = 1000.0,
    ...
    """
    Mixed derivative ∂²V_paper/(∂φ ∂T) evaluated at (φ, T).

    Computed by a symmetric finite difference in T applied to the high-order
    φ-derivative:

        ∂²V/(∂φ ∂T) ≈ [ ∂V/∂φ(φ, T + δT) - ∂V/∂φ(φ, T - δT) ] / (2 δT),

    where δT is chosen adaptively as

        δT = max(dT_min, dT_rel * |T|).

    Parameters
    ----------
    phi : array_like
        Field value(s) φ where the derivative is evaluated.
    T : float or array_like
        Temperature(s) in GeV; must satisfy T > 0 for all entries.
    C, Lambda, include_daisy, real_cont :
        Same physical meaning as in V_paper. The mixed derivative always refers
        to the full finite-temperature potential, so finiteT=True is assumed.
    dT_rel : float, optional
        Relative step-size in T used in the symmetric finite difference.
    dT_min : float, optional
        Absolute lower bound on δT (in GeV).

    Returns
    -------
    d2V_dphi_dT : ndarray
        Broadcasted shape of `phi` and `T`; mixed derivative ∂²V/(∂φ ∂T).
    """
    T_arr = np.asarray(T, dtype=float)
    if np.any(T_arr <= 0.0):
        raise ValueError(
            "d2V_paper_dphi_dT requires T > 0 for finite-temperature derivatives."
        )

    # Adaptive step in T: avoid extremely small δT
    dT = dT_rel * np.abs(T_arr)
    dT = np.maximum(dT, dT_min)

    if np.any(T_arr - dT <= 0.0):
        raise ValueError(
            "Temperature too small for symmetric finite difference with the chosen dT; "
            "consider lowering dT_rel/dT_min or evaluating at larger T."
        )

    # Symmetric finite difference in T of ∂V/∂φ, always for the full finite-T potential
    dphi_plus = dV_paper_dphi(
        phi,
        T=T_arr + dT,
        C=C,
        Lambda=Lambda,
    ...
    return (dphi_plus - dphi_minus) / (2.0 * dT)
...
def example_A_fig1_multiC(Cs=( -5.0, 0.0, 3.0, 3.7, 4.14, 5.0 ),
...
# Example C
# -----------------------------------------------------------------------------
def example_C_T_scan(C: float = 3.7,
                     Lambda: float = 1000.0,
                     T_list = (60, 80.0, 90, 100.0, 120.0),
                     phi_max: float = 300.0,
                     save_dir: Optional[str] = None,
                     shift_ref: str = "V0", tag: str = ""):
    """
...
# Example D
# -----------------------------------------------------------------------------
def example_D_potential_geometry(inst: SingleFieldInstanton,
                                 profile,
                                 save_dir: Optional[str] = None, tag: str = ""):
    """
    ...
    plt.tight_layout()
    suffix = f"_{tag}" if tag else ""
    savefig(fig1, save_dir, f"figD1{suffix}")
    ...
    plt.tight_layout()
    plt.show()
    savefig(fig2, save_dir, f"figD2{suffix}")

# -----------------------------------------------------------------------------
# Example E
# -----------------------------------------------------------------------------
def example_E_local_quadratic_at_phi0(inst: SingleFieldInstanton,
                                      profile,
                                      save_dir: Optional[str] = None, tag: str = ""):
    """
...
# Example F
# -----------------------------------------------------------------------------
def example_F_inverted_path(inst: SingleFieldInstanton,
                            profile,
                            save_dir: Optional[str] = None, tag: str = ""):
    """
    ...
    plt.show()
    suffix = f"_{tag}" if tag else ""
    savefig(fig1, save_dir, f"figF{suffix}")

# -----------------------------------------------------------------------------
# Example G
# -----------------------------------------------------------------------------
def example_G_phi_of_r(inst: SingleFieldInstanton,
                       profile,
                       save_dir: Optional[str] = None, tag: str = ""):
    """
...
# Example H
# -----------------------------------------------------------------------------
def example_H_spherical_maps(inst: SingleFieldInstanton,
                             profile,
                             save_dir: Optional[str] = None, tag: str = ""):
    """
...
# Example I
# -----------------------------------------------------------------------------
def example_I_ode_terms(inst: SingleFieldInstanton,
                        profile,
                        save_dir: Optional[str] = None, tag: str = ""):
    """
...
# Example J
# -----------------------------------------------------------------------------
def example_J_action_and_beta(inst: SingleFieldInstanton,
                              profile,
                              save_dir: Optional[str] = None,
    ...
    plt.tight_layout(); plt.show()
    suffix = f"_{tag}" if tag else ""
    savefig(fig, save_dir, f"figJ{suffix}")
...
def run_all(case: str = "paper",
    ...
    def VT_factory( C=C,
        ...
        return partial(V_paper, C=C, Lambda=Lambda, finiteT=finiteT, T=T, include_daisy=include_daisy,
                       real_cont=False, )


    inst, label = make_inst( V= VT_factory,
        case=case,
        phi_abs=phi_abs,
        phi_meta=phi_meta,
        C=C,
    ...
    example_A_fig1_multiC(Cs=(-5.0, 0.0, 3.0, 3.7, 4.14, 5.0),  Lambda=Lambda, phi_max=300.0, save_dir=save_dir,tag=tag,)
    ...
    ########################### Finite Temperature #####################################
    # C): Temperature sweep of V(φ,T) at fixed C (Fig.1-but now per T)
    example_C_T_scan(C=C, Lambda=Lambda, T_list=(60, 80.0, 90, 100.0, 120.0),
                     phi_max=300.0, save_dir=save_dir,
                     shift_ref="V0", tag=tag,)

    ########################## Bounce Solution #########################################

    # D) Potential geometry & inverted view with φ0
    example_D_potential_geometry(inst, profile, save_dir=save_dir, tag=tag,)

    # E) Local quadratic at φ0
    example_E_local_quadratic_at_phi0(inst, profile, save_dir=save_dir, tag=tag,)

    # F) Inverted potential with path, and V(φ) with points
    example_F_inverted_path(inst, profile, save_dir=save_dir, tag=tag,)

    # G) φ(r) with interior shading and markers
    example_G_phi_of_r(inst, profile, save_dir=save_dir, tag=tag,)

    # H) 2D spherical visualizations (Cartesian & polar)
    example_H_spherical_maps(inst, profile, save_dir=save_dir, tag=tag,)

    # I) ODE terms decomposition along the profile
    example_I_ode_terms(inst, profile, save_dir=save_dir, tag=tag,)

    # J) Action and β proxies (print β; only action is plotted)
    example_J_action_and_beta(inst, profile, save_dir=save_dir, tag=tag,)

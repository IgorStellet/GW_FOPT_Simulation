...
# ============================================================================
# Gravitational-wave helpers (using GravitationalWaveCalculator)
# ============================================================================

def _build_gw_calculator_from_summary(
    transition_summary: Mapping[str, Any],
) -> Optional[GravitationalWaveCalculator]:
    """
    Best-effort construction of a GravitationalWaveCalculator from the
    transitionFinder summary dictionary.

    It reuses:
      - V_XT(x, T)       : finite-T potential with V(0, T) subtracted;
      - dVdphi_XT(x, T)  : gradient wrt fields;
      - dVdT_XT(x, T)    : derivative wrt temperature;
      - phases           : dict of Phase objects;
      - main_transition  : to pick high/low phases.

    Returns None if any of the required pieces is missing.
    """
    try:
        V_XT = transition_summary["V_XT"]
        dVdphi_XT = transition_summary["dVdphi_XT"]
        dVdT_XT = transition_summary["dVdT_XT"]
        phases: Mapping[Hashable, Phase] = transition_summary["phases"]
    ...
    def V_for_gw(x: npt.NDArray[np.float64], T: float) -> float:
        # V_XT already returns a scalar for a single point (Ndim=1)
        return float(V_XT(np.asarray(x, dtype=float), float(T)))

    def dV_for_gw(x: npt.NDArray[np.float64], T: float) -> npt.NDArray[np.float64]:
        return np.asarray(dVdphi_XT(np.asarray(x, dtype=float), float(T)), dtype=float)

    def dVdT_for_gw(x: npt.NDArray[np.float64], T: float) -> float:
...
def _compute_gw_scales_from_calculator(
    gw_calc: GravitationalWaveCalculator,
    transition_summary: Mapping[str, Any],
    *,
    g_star: float,
    v_w: float,
    ...
    """
    Compute basic GW scales using ONLY methods from GravitationalWaveCalculator:

      - T_* (we take T_* = T_n from transitionFinder if available);
      - α(T_*);
      - β/H_* at T_*;
      - R_* H_*;
      - Γ(T_*);
      - peak frequencies for sound waves / turbulence / collisions.

    All numerical derivatives (for β/H_*) and κ-factors are handled internally
    by GravitationalWaveCalculator.
    """
    keyT = transition_summary.get("key_temperatures", {}) or {}
    T_star = float(keyT.get("Tn", np.nan))
    ...
    if not np.isfinite(T_star):
        # No sensible temperature to evaluate GW scales
        nan = float("nan")
        return {
            "gw_T_star_GeV": nan,
            "gw_alpha": nan,
    ...
    # Choose a finite-difference step dT that stays within the common phase range
    Tmin = float(gw_calc._T_min)
    Tmax = float(gw_calc._T_max)
    margin = min(T_star - Tmin, Tmax - T_star)
    if margin <= 0.0:
        # Degenerate overlap; fall back to a very small step
        dT = max(1e-3, dT_fraction * max(T_star, 1.0))
    else:
        dT_max = 0.001 * margin  # we need T ± 2 dT inside [Tmin, Tmax]
        dT_pref = dT_fraction * max(T_star, 1.0)
        dT = min(dT_pref, dT_max)
        dT = max(dT, 1e-4)

    # β/H_* via the dedicated helper (which internally uses dS_dT)
    beta_over_H = float(gw_calc.beta_over_H(T_star, dT))

    # Strength parameter α(T_*)
    alpha_val = float(gw_calc.alpha(T_star, g_star))

    # Dimensionless bubble size R_* H_*
    R_star_H = float(gw_calc.bubble_radius_over_H(beta_over_H, v_w=v_w))

    # Nucleation rate Γ(T_*)
    Gamma_val = float(gw_calc.nucleation_rate(T_star))

    # Peak frequencies for each component
    try:
        f_sw_peak = float(gw_calc._f_sw_peak(
    ...
    try:
        f_turb_peak = float(gw_calc._f_turb_peak(
            beta_over_H=beta_over_H,
            T_star=T_star,
            g_star=g_star,
            v_w=v_w,
    ...
    try:
        f_coll_peak = float(gw_calc._f_coll_peak(
            beta_over_H=beta_over_H,
            T_star=T_star,
            g_star=g_star,
            v_w=v_w,
    ...
    return {
        ...
        "gw_alpha": alpha_val,
        "gw_beta_over_H": beta_over_H,
        "gw_R_star_times_H": R_star_H,
        "gw_nucleation_rate_GeV4": Gamma_val,
        "gw_dT_for_beta_GeV": float(dT),
        "gw_f_sw_peak_Hz": f_sw_peak,
        "gw_f_turb_peak_Hz": f_turb_peak,
        "gw_f_coll_peak_Hz": f_coll_peak,
        "gw_g_star": float(g_star),
        "gw_v_w": float(v_w),
    }
...
def gather_diagnostics(
    ...
    # ---- Extra: transitionFinder data (Block C + D) ----
    if transition_summary is not None:
        ...
        # ---- GW scales (via GravitationalWaveCalculator) ----
        # Prefer cached values from Example O, if present
        gw_scales = transition_summary.get("gw_scales")
        ...
        if gw_scales is None:
            ...
            if gw_calc is not None:
                ...
                transition_summary["gw_scales"] = gw_scales  # cache
...
def run_all(case: str = "paper",
    ...
    # O) Gravitational Waves from bounce

...
from CosmoTransitions.gravitational_Waves import  lisa_sensitivity_s_pis, decigo_sensitivity_s_pis, bbo_sensitivity_s_pis
...
from CosmoTransitions.generic_potential import (  build_finite_T_derivatives,
    ensure_dir, savefig, build_phi_grid, _extract_params_from_V, _build_phases_and_transitions, _spinodal_data_for_phase,
    _closest_spinodal_to_T, _build_gw_calculator_from_summary, _compute_gw_scales_from_calculator,
...
def example_A_fig1_multiVEV(
    ...
    for _VEW in VEWs:
        V0 = V_paper(0.0, finiteT=False, vev=_VEW)
        Vn = V_paper(phi_axis, finiteT=False, vev=_VEW) - V0
        plt.plot(phi_axis, Vn, lw=2.0, label=f"v = {_VEW:g} GeV")
...
def example_B_paper_potential_with_inset(
    ...
    ax.plot(φ, V_shift, lw=2.2, label=rf"$VEW={_VEW:.0f}$ GeV")
...
def example_G_T_scan(
    ...
    *,
    ...
    ax.set_title(fr"Temperature sweep (no measure deformation), $VEW={_VEW:.0f}$ GeV")
    ax.grid(True, alpha=0.25)
...
def run_all(case: str = "paper",
    ...
    example_A_fig1_multiVEV(phi_max=300.0, save_dir=save_dir,tag=tag,)
    ...
    def make_V_phi_only(T: float = T_bounce,
                        finiteT=finiteT,
                        include_daisy=include_daisy):
        """
    ...
    save_diagnostics_summary(di, save_dir, basename=f"diagnostics_summary_{_VEW}", fmt="json")
...
if __name__ == "__main__":
    ...
    run_all(
        ...
        Tn_Ttol= 1e-3,
        Tn_maxiter= 80,
        save_dir=f"results_VEW_{_VEW}",        # "results"
    )
